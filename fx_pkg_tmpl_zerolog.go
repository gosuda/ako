...
func createFxZerologFile(path string, name string) error {
	...
	if err := writeTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), zerologWriterTemplate, map[string]any{
	...
	if err := getGoModule(zerologDependency); err != nil {
