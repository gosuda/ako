...
func createFxDuckDBFile(path string, name string) error {
	...
	if err := writeTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), duckdbClientTemplate, map[string]any{
	...
	if err := getGoModule(duckdbDependency); err != nil {
