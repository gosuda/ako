...
*   `**/pkg/**`: Contains the **concrete implementations** of the interfaces defined in `lib`. All infrastructure-related code resides here. 
    *   A `pkg` implementation must be a self-contained, minimal implementation of a single `lib` interface (e.g., implementing the `Repository` interface from the `lib/repository/user` package using `sqlc` and `pgx`).
    ...
    *   **Client Wrapping Pattern for Multiple Interfaces**: When a single technology package (e.g., `pkg/cache/redis`) needs to implement multiple `lib` interfaces (e.g., `user.Cache`, `room.Cache`), a wrapping pattern **must** be used.
        ...
        2.  Each `lib` interface implementation should be in its own file (e.g., `user_cache.go`, `room_cache.go`).
        3.  Each implementation struct will wrap the core client (e.g., `type UserCache struct { client *Client }`) and use it to implement the specific interface methods. This promotes code reuse and clear separation of concerns within the package.
