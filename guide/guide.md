# Go Project Engineering Guide

This document defines a standard, production-ready package structure and a set of engineering principles for building robust, flexible, and scalable Go applications. Its purpose is to ensure consistency, quality, and productivity across the team.
...
* `**/internal/**`: Contains the application's business logic (`controller`, `service`).
    ...
    * `**internal/service/**`: Executes core business logic and orchestrates use cases by combining interfaces from `lib`.
...
* `**/pkg/**`: Contains the **concrete implementations** of the interfaces defined in `lib`. All infrastructure-related code resides here. Unit tests for each implementation must be located within the same package.
...
#### Database Transaction Management (Unit of Work)

The **Unit of Work (UoW)** pattern is strongly recommended for managing database transactions. This ensures that a business operation involving multiple data changes is executed as a single atomic unit, preserving data integrity.

#### Asynchronous Processing & Event-Driven Architecture

For long-running tasks or decoupling systems, an event-driven approach is recommended.
* **Message Queues**: Use message queues like **NATS, Kafka, or Pulsar**.
* **Data Consistency**: The **Outbox Pattern** should be adopted to ensure atomic consistency between database transactions and message publishing.

#### DTO (Data Transfer Object) Strategy

To maintain strict layer separation, each layer should use its own dedicated DTOs.
* **Controller DTOs**: In `internal/controller/{feature}` for API requests/responses.
* **Service DTOs**: In `internal/service/{feature}` for business logic inputs/outputs.
* **Repository DTOs**: In `lib/repository/{feature}` for database interactions.
...
* `cmd/publicapi/` → `app.go`
* `internal/controller/userapi/` → `controller.go`
* `internal/service/membership/` → `service.go`
* `pkg/client/postgres/` → `client.go`
* `lib/repository/users/` → `users.go`