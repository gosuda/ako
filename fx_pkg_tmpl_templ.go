...
func createFxTemplFile(path string, name string) error {
	...
	if _, err := os.Stat(objectFilePath); os.IsNotExist(err) {
		if err := writeTemplate2File(objectFilePath, templObjectTemplate, map[string]any{
	...
	if _, err := os.Stat(generatorFilePath); os.IsNotExist(err) {
		if err := writeTemplate2File(generatorFilePath, templGeneratorTemplate, map[string]any{
			"package_name": packageName,
			"client_name":  name,
	...
	if err := getGoModule(templDependency); err != nil {
		return fmt.Errorf("getGoModule: %w", err)
	}

	if err := getGoModule(templToolDependency); err != nil {
		return fmt.Errorf("getGoModule: %w", err)
	}
