...
func createFxGrpcClientFile(path string, name string) error {
	...
	if err := writeTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), grpcClientTemplate, map[string]any{
	...
	if err := getGoModule(grpcClientDependency); err != nil {
		return fmt.Errorf("getGoModule: %w", err)
	}

	if err := getGoModule(grpcClientDependencyProtobuf); err != nil {
		return fmt.Errorf("getGoModule: %w", err)
	}
