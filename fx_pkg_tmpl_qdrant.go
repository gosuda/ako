...
func createFxQdrantFile(path string, name string) error {
	...
	if err := writeTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), qdrantClientTemplate, map[string]any{
	...
	if err := getGoModule(qdrantDependency); err != nil {
