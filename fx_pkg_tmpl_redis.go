...
func createFxRueidisFile(path string, name string) error {
	...
	if err := writeTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), rueidisClientTemplate, map[string]any{
	...
	if err := getGoModule(rueidisDependency); err != nil {
