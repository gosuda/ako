...
func createFxMSSQLFile(path string, name string) error {
	...
	if err := template.WriteTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), mssqlClientTemplate, map[string]any{
	...
	if err := template.WriteTemplate2File(filepath.Join(path, "mssql_connection_string.go"), mssqlConnectionStringTemplate, map[string]any{
		"package_name": packageName,
	}); err != nil {
		return err
	}

	if err := module.GetGoModule(mssqlDependency); err != nil {
