...
func createFxNatsFile(path string, name string) error {
	...
	if err := template.WriteTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), natsClientTemplate, map[string]any{
	...
	if err := module.GetGoModule(natsDependency); err != nil {
		return fmt.Errorf("getGoModule: %w", err)
	}

	if err := module.GetGoModule(natsDependencyErrgroup); err != nil {
		return fmt.Errorf("getGoModule: %w", err)
	}
