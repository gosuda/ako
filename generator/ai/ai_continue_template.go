...
const (
	aiContinueTemplateFileName = ".continue/prompts/commit-message-generator.prompt"
	aiContinueTemplate         = `name: generate-conventional-commit-messages
description: Generate commit messages that follow the Conventional Commits specification.
---
## LLM Prompt: Generate Conventional Commit Messages
...
func CreateVsCodeContinueSettings() error {
	if err := os.MkdirAll(filepath.Dir(aiContinueTemplateFileName), os.ModePerm); err != nil {
		return err
	}

	if err := os.WriteFile(aiContinueTemplateFileName, []byte(aiContinueTemplate), 0644); err != nil {
		return err
	}
