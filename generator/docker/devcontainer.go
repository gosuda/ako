...
`
	...
	if err := template.WriteTemplate2File(path, t, map[string]any{
		"cmd_name": appName,
		"app_name": k8s.MakeCmdDepthToName(strings.Split(appName, "/")...),
	}); err != nil {
		return err
	}
...
`
	...
	if err := template.WriteTemplate2File(path, goImageTemplate, nil); err != nil {
		return err
	}
	...
	if err := template.WriteTemplate2File(path, devcontainerJsonTemplate, map[string]any{
		"name": name,
	}); err != nil {
		return err
	}
...
`
	...
	if err := template.WriteTemplate2File(path, dockerComposeTemplate, map[string]any{
		"name": name,
	}); err != nil {
		return err
	}
