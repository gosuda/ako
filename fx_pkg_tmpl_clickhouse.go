...
func createClickhouseStructFile(path string, name string) error {
	...
	if err := writeTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), clickhouseClientTemplate, map[string]any{
	...
	if err := getGoModule(clickhouseDependency); err != nil {
