...
func createFxVaultFile(path string, name string) error {
	...
	if err := writeTemplate2File(filepath.Join(path, fmt.Sprintf(fxFileName, name)), vaultClientTemplate, map[string]any{
	...
	if err := writeTemplate2File(filepath.Join(path, "vault_objects.go"), vaultObjectTemplate, map[string]any{
		"package_name": packageName,
		"client_name":  name,
	}); err != nil {
		return err
	...
	if err := getGoModule(vaultDependency); err != nil {
